...
if ($_SERVER['REQUEST_METHOD'] == 'GET'){
	  ...
	  if (!empty($_COOKIE['save'])) {
		...
		if (!empty($_COOKIE['pass'])) {
			$messages['passmessage'] = sprintf('You can <a href="login.php">log in</a> with login <strong>%s</strong>
			  and password <strong>%s</strong> for changing data.',
      ...
      session_start() && !empty($_SESSION['login'])){
		...
		try {
			$id = $_SESSION['uid'];
			$pdostate = $db->prepare("SELECT name,email,birthdate,sex,bio FROM application6 WHERE id=:id");
...
$mailregex = "/^[\w\.-]+@([\w-]+\.)+[\w-]{2,4}$/";
$super_list = array('Pascal','C','CPP', 'Python');
...
// кроме логина и пароля.
	try{
		$stmt = $db->prepare("UPDATE application6 SET name=:name, email=:email, birthdate=:birthdate, sex=:sex, bio=:bio WHERE id=:this_id");
		...
		$spped = $db->prepare("DELETE FROM Languages6 WHERE id=:this_id");
		...
		$sppe= $db->prepare("INSERT INTO Languages6 SET name=:name, id=:person");
		$sppe->bindParam(':person', $_SESSION['uid']);
		foreach($languages as $inserting){
	...
	try {
	$stmt = $db->prepare("INSERT INTO application6 SET name=:name, email=:email, birthdate=:birthdate, sex=:sex, bio=:bio");
	$stmt->bindParam(':name', $name);
	$stmt->bindParam(':email', $email);
	$stmt->bindParam(':birthdate', $birth);
	$stmt->bindParam(':sex', $sex);
